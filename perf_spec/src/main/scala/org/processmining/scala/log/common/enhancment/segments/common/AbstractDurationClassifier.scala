...
class OutlierClassifier extends AbstractDurationClassifier {

  def classify(duration: Long, q2: Double, median: Double ,q4: Double, caseId: String, timestamp: Long, segmentName: String, medianAbsDeviation:Double): Int =
    duration match {
      case n if 30 > ((n-median)*0.6745)/medianAbsDeviation => 0
      case _ => 1
    }

  override def sparkSqlExpression(attrNameDuration: String, attrNameClazz: String) =
    s"""SELECT segments.*,
       | CASE WHEN (segments.${attrNameDuration} -stat.median) * 0.6745 / stat.medianAbsDeviation > 30 THEN 1
       | ELSE 0 END END
       | AS ${attrNameClazz}
       | FROM segments
       | JOIN stat ON stat.key == segments.key""".stripMargin

  override val legend = "DURATION%Not_Outlier%Outlier"
