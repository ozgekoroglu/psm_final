...
class PreProcessor(filename: String,
                   ...
                   handler: Runnable,
                   customClassifierName: String) extends Callable[String] {
  ...
  override def call(): String = {
    ...
    SegmentsToSpectrum.start(files,
      ...
      twCount,
      spectrumDirName,
      PreProcessor.durationClassifier(durationClassifierCode, customClassifierName, filename))
    spectrumDirName
  }
}
...
object PreProcessor {
  ...
  def durationClassifier(code: Int, className: String, path: String): AbstractDurationClassifier =
    code match {
