...
class EventVarExpressionTest_DFR_ContainsNoVars extends FunSuite {
  ...
  test("testContains_AtTheEnd") {
    val trace = TestUtils.create("0123456789", DefaultTraceId)
    val ex = EventVar("8") >> EventVar("9")
    assert(ex.contains(trace))
  }

  test("testContains_AtTheEndNotMatching_1") {
    val trace = TestUtils.create("0123456789", DefaultTraceId)
    val ex = EventVar("8") >> EventVar("x")
    assert(!ex.contains(trace))
  }

  test("testContains_AtTheEndNotMatching_2") {
    val trace = TestUtils.create("0123456789", DefaultTraceId)
    val ex = EventVar("9") >> EventVar("x")
    assert(!ex.contains(trace))
  }
}
